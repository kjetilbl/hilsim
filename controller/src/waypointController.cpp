...
double WaypointController::deg2rad(double degrees){
	return degrees*M_PI/180;
}

double WaypointController::distance_m(gpsPoint a, gpsPoint b)
{
	...
	if ( h >= 1 )
		std::cout << "Warning: in function distance_m(), h >= 1, which might give floating point error." << std::endl;
	double r = 6378000; // meters, assumed constant
	double d = 2*r*asin(sqrt(h));
	return abs(d);
}

double WaypointController::compass_bearing(gpsPoint from, gpsPoint to){
...
double WaypointController::latitude_degs_pr_meter()
{
	double r = 6378000; // meters, assumed constant
	return 90/(2*M_PI*r/4);
}

double WaypointController::longitude_degs_pr_meter(double currentLatitude)
{
	double deg2rad = M_PI/180;
	double r = 6378000; // meters, assumed constant
